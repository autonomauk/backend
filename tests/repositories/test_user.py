...
def test_type_checking(user: User):
...
def test_add_tracks_to_log(user: User):
    UserRepository.create(user)
...
def test_read_track_log(user: User):
    UserRepository.create(user)
    ...
    tracks, total = UserRepository.read_track_log(user.id, 0, int(2**8))
